...
const _NOTICE_PROMPT = `你是星宇航空(STARLUX Airlines)飛行員助理。請分析以下公告，以純 JSON 回應（不加任何 markdown 或說明文字）：
{
  ...
  "source_tag": "fleet_notice 或 message 或 email（來源類型：FN-XX-XXXX 正式文件、FTS/GTSM 等內部官方通知 → fleet_notice；Teams/Line 訊息 → message；Outlook/Email → email）",
  "tags": ["ops/safety/manual_update/admin/app_notice 中一或多個內容標籤（ops：飛行操作/計畫/地面作業；safety：安全警示；manual_update：手冊程序修訂；admin：行政/人事；app_notice：App 版本）"],
