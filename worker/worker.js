...
const _NOTICE_PROMPT = `你是星宇航空(STARLUX Airlines)飛行員助理。請分析以下公告，以純 JSON 回應（不加任何 markdown 或說明文字）：
{
  ...
  "category": "fleet_notice 或 ops 或 safety 或 manual_update 或 admin 或 app_notice（分類規則：FN-XX-XXXX 編號的正式機隊公告文件 → fleet_notice；涉及手冊/程序修訂 → manual_update；安全警示 → safety；飛行計畫/地面作業/特定航班通告 → ops；人事/行政/薪資 → admin；App 版本 → app_notice）【強制規則】來源為 Teams、Line、Outlook、Email 的公告，category 絕對不可以是 fleet_notice，無論內容為何",
