...
{
  "title": "一句話標題（30字內）",
  "issue_date": "公告的發佈/發出日期 YYYY/MM/DD。優先順序：①FN文件上的 'Issue Date' 欄位 ②表格截圖中該筆公告所在列的第一欄日期 ③Teams/Outlook 郵件/訊息的發送日期。不是生效日(Effective Date)。找不到才填 null",
...
注意：
- manual_update 類別：summary 必須如實列出該次更新的具體內容，例如「FCOM 2.04.10 新增 xx 限制」「QRH 修改 yy 步驟為…」；若原文已有條列更新重點，直接照抄重點文字，絕對不能只寫「手冊更新」或「有修訂」等空泛描述
...
每列都要提取，不要遺漏。若只有一筆也輸出含一個元素的陣列。只輸出 JSON 陣列。`;
...
async function handleRequest(request, env) {
  ...
  try {
    ...
    if (url.pathname === '/api/notices/upload' && request.method === 'POST') {
      ...
      const list = await _noticesGet(env);
      if (_isDuplicate(notice, list)) {
        return new Response(JSON.stringify({ ok: false, skipped: true, reason: '重複公告已略過', title: notice.title }), {
          headers: { ...headers, 'Content-Type': 'application/json' },
        });
      }
      list.unshift(notice);
