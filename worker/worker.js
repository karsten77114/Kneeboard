...
async function handleRequest(request, env) {
  ...
  try {
    ...
    if (url.pathname === '/api/notices/upload' && request.method === 'POST') {
      ...
      const _SOURCE_HINT_MAP = {
        'email': 'email', 'outlook': 'email', 'mail': 'email',
        'message': 'message', 'teams': 'message', 'line': 'message', 'msg': 'message',
        'fleet_notice': 'fleet_notice', 'fn': 'fleet_notice', 'fleet': 'fleet_notice', 'fts': 'fleet_notice',
      };
      const _mappedSourceTag = _SOURCE_HINT_MAP[(sourceHint || '').toLowerCase().trim()];
